...
To merge in new changes from upstream, the following steps need to be performed:

* `git pull` in the submodule
* run the `scripts/extract-upstream-version.py` helper to re-generate the version shadow
  file in the `local/` tree
* `git add` the changes (submodule and `local/`)
